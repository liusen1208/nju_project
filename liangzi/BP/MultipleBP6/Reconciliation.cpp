...
void calNumOfRL(){                      
	...
	for(i=0;i<n;i++){      //遍历VN
		...
		cn=Dv6[i];               //该VN度 (Dv已经赋初值)
		for(k=0;k<cn;k++){       //遍历与该VN相连的CN 
			j=Hl6[i][k].N;              //与VN i相连的CN j 
			tmp = 0;
			while(Hr6[j][tmp].N != i) tmp++;  //找出VN i为第几个与CN j相连VN 
			Hl6[i][k].numOfRL=tmp;      //求出numOfRL值           
		} 
	}
	...
	for(j=0;j<m;j++){      //遍历CN
		...
		vn=Dc6[j];                //该CN度 (Dc已经赋初值)
		for(k=0;k<vn;k++){       //遍历与该CN相连的VN 
			i=Hr6[j][k].N;              //与CN j相连的VN i
			tmp = 0;
			while(Hl6[i][tmp].N != j) tmp++;  //找出CN j为第几个与VN i相连CN 
			Hr6[j][k].numOfRL=tmp;      //求出numOfRL值           
		} 
	}
}
				...
				if(Num==4){
					...
					if(Num==5){
						...
						while (ifile.getline(buff, 50, ' ')) {//读出行 
							...
							ifile.getline(buff, 50, '\n');        //读出列 
							column = atoi(buff);                  //转成整型
							ed.N = column; Hr6[row].push_back(ed); //放到Hr,Hl 
							ed.N = row; Hl6[column].push_back(ed);
							Dc6[row]++;                            //该CN的度+1
							Dv6[column]++;                         //该VN的度+1
						}
					}			
...
/*传统BP算法，返回协商是否正常结束*/
int MBP6(double e,int iteraM){   //误码率，迭代次数 
	...
	for(j=0;j<m;j++){       //遍历每个CN 
		...
		for(i=0;i<vn;i++){//遍历与该CN相连VN 
		    tmpe=Hr6[j][i].N;                  //VN
		    Hr6[j][i].q0=P[tmpe][0];           //q tmpe_j(0)
			Hr6[j][i].q1=P[tmpe][1];           //q tmpe_j(1)
			k=Hr6[j][i].numOfRL;
			Hl6[tmpe][k].q0=Hr6[j][i].q0;       //q tmpe_j(0)
			Hl6[tmpe][k].q1=Hr6[j][i].q1;       //q tmpe_j(1)
		}
	}
	...
	do{
		...
		for(j=0;j<m;j++){                 //遍历每个CN
			...
			kk=0;		
			for(tmp1=1.0,k=0;k<vn;k++)          //计算公式中一个乘积
				if(Hr6[j][k].q1!=0.5)  tmp1=tmp1*(1.0-2*Hr6[j][k].q1); 
				else kk++;	 			 
			for(i=0;i<vn;i++){                  //遍历这些VN 				
				tmpe=Hr6[j][i].N;                          //该VN 
				if(kk==0){
					tmp0=tmp1/(1.0-2*Hr6[j][i].q1); 
				}else{
					if(kk==1){
						if(Hr6[j][i].q1==0.5){
							tmp0=tmp1;
				...
				Hr6[j][i].r0=(Z6[j]==1)?(0.5-0.5*tmp0):(0.5+0.5*tmp0);//r j_tmpe(0) 
				Hr6[j][i].r1=1.0-Hr6[j][i].r0;              //r j_tmpe(1) 
				k=Hr6[j][i].numOfRL;		                      
				Hl6[tmpe][k].r0=Hr6[j][i].r0;               //r j_tmpe(0)
				Hl6[tmpe][k].r1=Hr6[j][i].r1;               //r j_tmpe(1)		
			}
		}
		...
		for(i=0;i<n;i++){                 //遍历每个VN
			...
			cn=Dv6[i];                     //与该VN相连CN数  
			tmp[6][0]=1.0; tmp[6][1]=1.0;   //计算公式中两个乘积
			for(k=0;k<cn;k++){  
				tmp[6][0]=tmp[6][0]*Hl6[i][k].r0;
				tmp[6][1]=tmp[6][1]*Hl6[i][k].r1;
			}				
			
			tmp0=P[i][0]*tmp[1][0]*tmp[2][0]*tmp[3][0]*tmp[4][0]*tmp[5][0]*tmp[6][0];
			tmp1=P[i][1]*tmp[1][1]*tmp[2][1]*tmp[3][1]*tmp[4][1]*tmp[5][1]*tmp[6][1];
			...
			cn=Dv6[i];
			for(j=0;j<cn;j++){
				tmp3=P[i][0]*tmp[6][0]/Hl6[i][j].r0;	tmp4=P[i][1]*tmp[6][1]/Hl6[i][j].r1;	
				Hl6[i][j].q0=((tmp3+tmp4)==0.0)?0.5:((1.0/(tmp3+tmp4))*tmp3);//q i_tmpe(0)	              
				Hl6[i][j].q1=1-Hl6[i][j].q0;            //q i_tmpe(1)
				tmpe=Hl6[i][j].N;  k=Hl6[i][j].numOfRL;                                 
				Hr6[tmpe][k].q0=Hl6[i][j].q0;           //q i_tmpe(0)
				Hr6[tmpe][k].q1=Hl6[i][j].q1;           //q i_tmpe(1)	
			}		                             
		}	
...
void getZ(){
	...
	for (j = 0;j<m;j++) {           //计算H*X		
		...
		vn=Dc6[j]; k=0;
		for(i=0;i<vn;i++){          //遍历与 CN j相连的所有 VN 
			if(X[Hr6[j][i].N]==1) k++;     //H第j行与Y对应位相乘为 1 的个数加1 
		}	
		Z6[j]=((k%2)==1)?1:0;        //H第j行与Y相乘(为得到0/1结果，*/+分别用&&/^代替)	
	}	
} 
